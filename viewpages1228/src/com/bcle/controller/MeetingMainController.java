...
public class MeetingMainController implements Controller
{
	...
	public ModelAndView handleRequest(HttpServletRequest request, HttpServletResponse response) throws Exception
	{
		...
		try
		{
			...
			checkClubMem = dao.checkClubMember(id, cid);
			checkClubJoin = dao.checkClubJoin(id, cid);
			checkClubCap = dao.checkClubCap(id, cid);
			clubJoinList = dao.joinClubmemList(cid);
			//System.out.println(checkClubJoin+":"+checkClubCap);
			
			mav.addObject("clubInfo", clubInfo);
			mav.addObject("cid", cid);
			mav.addObject("b", b);
			mav.addObject("cntMem", cntMem);
			mav.addObject("checkClubMem", checkClubMem);
			mav.addObject("checkClubJoin", checkClubJoin);
			mav.addObject("checkClubCap", checkClubCap);
